...
export async function maxAuthSpotifyRequest<T = unknown>(userId: string, path: string, options: RequestOptions = {}): Promise<T> {
  ...
  const url = env.MAX_AUTH_INTERNAL_URL.replace(/\/$/, '') + '/users/' + encodeURIComponent(userId) + '/spotify' + path + queryString(options.query);
  ...
  if (!response.ok) {
    throw new ApiError(
      response.status,
      body?.error?.code || body?.code || 'MAX_AUTH_SPOTIFY_ERROR',
      body?.error?.message || body?.message || 'MAX Auth Spotify request failed'
    );
  }
